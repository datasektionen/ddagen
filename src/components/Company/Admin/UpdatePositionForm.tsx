...
export function UpdatePositionForm({
    ...
    closeModal,
    setPosition,
    setShowUpdatePositionForm
} : {
    t: Locale;
    exhibitor: Exhibitor;
    closeModal: () => void;
    setPosition: (z: string, mapPosition: number) => void;
    setShowUpdatePositionForm: (a: boolean) => void;
}) {
    const modalRef = useRef<HTMLDivElement>(null);

    const [error, setError] = useState("");

    const [mapPositionString, setMapPositionString] = useState<string>(exhibitor?.mapPosition?.toString() ?? "");
            ...
            <div className="flex flex-col px-5 mt-5">
              ...
              <form
                ...
                onSubmit={(e) => {
                    ...
                    setPosition(
                      exhibitor.id,
                      parseInt(mapPositionString) ?? 0,
                  )
                }}
              >
                  <h3 className="text-lg font-medium mt-2">Plats:</h3>
                  <div className="flex flex-col gap-3">
                    <InputField
                    type="text"
                    name="name"
                    value={mapPositionString}
                    setValue={setMapPositionString}
                    fields={t.exhibitorSettings.fieldsUpdatePosition}
                    dark={true}
                    />
                  </div>
