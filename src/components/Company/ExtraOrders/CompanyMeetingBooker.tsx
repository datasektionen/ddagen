                  ...
                  <td className="px-8">
                    <div>
                      <button className="my-1" onClick={
                        () => {
                        if(data.has_cv) loadAndOpenPdf(data.ugkthid);
                          //openPDF(data.cv);
                        }
                      }>
                        <a className={`block transition-transform rounded-full text-white text-base font-medium px-4 py-1 max-lg:mx-auto w-max 
                            ${(data.has_cv  ? ' bg-cerise hover:scale-105' : ' bg-gray/50 pointer-events-none')}`
                            //cv.startsWith("data:application/pdf;base64,") 
                        }>
                          CV       
                        </a>
                      </button>
