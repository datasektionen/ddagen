            ...
            <div className="flex flex-row justify-between w-full px-[100px] mt-[200px]">
                {packets.map((packet, i) => <div className="flex flex-col">
                    <div className={`relative z-40 flex flex-col py-[30px] px-10 ${packetColor1[i]} border-[3px] rounded-xl bg-slate-50 bg-opacity-10 items-center`}>
                        ...
                        <button onClick={() => setDrop(!dropped)} className={`absolute h-[40px] w-[100px] ${packetColor2[i]} rounded-[40px] border-cerise bottom-0 translate-y-1/2`}>
                            <i className={`${dropped ? "" : "rotate-180"} duration-200 fas fa-chevron-up text-white`}></i>
                        </button>
                    </div>
                    <div className={`
                    ${dropped ? " max-h-full py-[40px]"
                    ...
                    {packet.map((row, j) => (
                            <div className={`flex flex-col py-2 items-center`}>
                                <i className={`${dropped ? "text-4xl" : "text-[0px]"} fas ${icons[i][j]} duration-200 text-white drop-shadow-[0_1.2px_1.2px_rgba(0,0,0,0.8)]`}></i>
