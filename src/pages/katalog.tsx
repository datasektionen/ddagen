        ...
        <div className="pt-[200px] pb-[300px]">
            ...
            <div className="flex flex-row justify-between px-[10px] md:px-[80px] mt-[150px] overflow-x-auto overflow-y-hidden pb-6">
                {packets.map((packet, i) => <div className="flex flex-col px-[20px] md:px-[10px]">
                    ...
                    px-[0px] transition-[max-height,padding] duration-200 overflow-y-hidden z-0 flex flex-col ${packetColor1[i]} border-[3px] border-t-0 rounded-b-xl bg-opacity-10 text-center bg-slate-50 `}>
                    {packet.map((row, j) => (
                            <div className={`flex flex-col py-2 items-center px-[50px] mt-10`}>
                            <i className={`text-2xl fas ${icons[i][j]} text-white`} 
